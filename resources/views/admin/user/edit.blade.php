...
@section('content')
    <div class="bg-white border border-coffee-bean/10 rounded-2xl p-6 md:p-8 shadow-sm space-y-6">
        ...
        <div class="flex items-center space-x-3 border-b border-coffee-bean/10 pb-5">
            ...
            <div>
                <h2 class="text-xl font-extrabold tracking-tight text-coffee-bean">Ubah Pengguna</h2>
                <p class="text-xs opacity-65">Ubah rincian akun administrator yang ada</p>
            </div>
        </div>

        <!-- Form -->
        <form action="{{ route('admin.user.update', $user->id) }}" method="POST" class="max-w-xl space-y-6">
            @csrf
            @method('PUT')
            ...
            <!-- Password Field -->
            <div>
                <label for="password" class="block text-sm font-semibold mb-2 text-coffee-bean">Kata Sandi Baru
                    (Opsional)</label>
                <div class="relative">
                ...
                <p class="mt-1.5 text-[10px] text-coffee-bean/60 leading-relaxed">
                    * Kosongkan kolom kata sandi jika Anda hanya ingin memperbarui nama atau alamat email pengguna ini.
                </p>
                @error('password')
                    <p class="mt-2 text-xs text-black-cherry font-medium">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <div class="relative">
                    <input id="password_confirmation" type="password" name="password_confirmation"
                        class="w-full pr-12 px-4 py-3 bg-frosted-blue/20 border border-coffee-bean/10 rounded-xl focus:border-coffee-bean/40 focus:ring-1 focus:ring-coffee-bean/40 outline-none transition text-sm text-coffee-bean placeholder-coffee-bean/40"
                        placeholder="Ulangi kata sandi baru jika ingin mengganti">
