        ...
        <form action="{{ route('admin.product.update', $product->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div>
                ...
                <div id="editor-deskripsi"
                    class="bg-frosted-blue/20 border border-coffee-bean/10 rounded-xl overflow-hidden text-sm text-coffee-bean">
                    {!! old('deskripsi', $product->deskripsi) !!}
                </div>
                <textarea id="deskripsi" name="deskripsi" class="hidden">{{ old('deskripsi', $product->deskripsi) }}</textarea>
                @error('deskripsi')
                    <p class="mt-2 text-xs text-black-cherry font-medium">{{ $message }}</p>
                @enderror
            </div>
